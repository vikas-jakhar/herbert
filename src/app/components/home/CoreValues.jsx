        ...
        <div className='relative py-20'>
            <div className="container py-10">
                ...
                <div className="mt-14 relative flex flex-col items-center gap-11">
                    <Image src='/assets/images/webp/work-tourism.png' className='w-full max-w-[286px] lg:max-w-[259px] lg:absolute lg:top-1/2 lg:left-1/2 lg:-translate-y-1/2 lg:-translate-x-1/2' alt='work-tourism' width={100} height={100} />
                            ...
                            <div key={i.id} className={`flex flex-row ${i.id === 2 ? " mt-[33px]" : ""} items-start lg:gap-[299px] gap-6 xl:gap-0 justify-between h-full w-full relative z-[1]`}>
                                {i.items.map((e, index) => (
                                    <div key={index} className={`flex max-w-[378px] flex-col ${index === 0 ? "lg:items-end items-start" : "items-start"} ${index === 1 ? "lg:hidden" : ""} ${i.id === 1 && index === 2 ? "hidden lg:flex" : ""} ${i.id === 2 && index === 0 ? "hidden lg:flex" : ""}`}>
                                        <span className='grid place-items-center text-white font-semibold md:text-lg text-xs md:w-10 md:h-10 w-[30px] h-[30px] bg-offOrange rounded-full'>{e.number}</span>
                                        <p className={`font-medium md:text-xl text-base text-offBlack md:mt-4 mt-2 ${index === 0 ? "lg:text-right text-left" : "text-left"}`}>{e.heading}</p>
                                        <PrimaryParagraph className={`mt-3 ${index === 0 ? "lg:text-right text-left" : "text-left"}`} children={e.text} />
                                    </div>
                                ))}
                            </div>
                        ))}
